...
function banBackSpace(e){
	var ev = e || window.event;//获得event对象
	var obj = ev.target||ev.srcElement;//获得事件源
	var t = obj.type||obj.getAttribute('type');
	var flag = (ev.keyCode == 8 && t!="password" && t!="text" && t!="textarea")?true:false;
	if(flag){return false;}
	}
document.onkeypress= banBackSpace;//FF opera
document.onkeydown=banBackSpace;//IE Chrome

function banEnter(e){
	var ev = e || window.event;//获得event对象
	var obj = ev.target||ev.srcElement;//获得事件源
	var t = obj.type||obj.getAttribute('type');
	var flag = (ev.keyCode == 13 && t!="textarea")?true:false;
	if(flag){return false;}
	}
document.onkeypress= banEnter;//FF opera
document.onkeydown=banEnter;//IE Chrome

var IMAGESPATH = '../img/popup/'; //图片路径配置
