...
class Image
{
    ...
    static function img($file, $w=0, $h=0, $x = 0, $y = 0)
    {
        if (empty($file)) {
        ...
        $hash = md5($string);
        $filename = self::getPhotoName($hash);
        ...
        try {
            $img_r = imagecreatefromstring($string);
        ...
        return str_replace(ROOT . '/web', '', $filename);
    }

    static function photo($file, $w, $h, $x = 0, $y = 0)
    {
        if (empty($file)) {
        ...
        $string = file_get_contents($file);
        $filename = self::getPhotoName(md5($string));
    ...
    static function getPhotoName($hash, $ext = '.jpg')
    {
        $path = ROOT . '/web/upload/photo';

        return self::getName($hash, $path);
    }

    static function getImgName($hash, $ext = '.jpg')
    {
        $path = ROOT . '/web/upload/photo';
    ...
    static function getName($hash, $path, $ext = '.jpg')
    {

        $dir = $path . '/' . substr($hash, 0, 1) . '/' . substr($hash, 1, 1) . '/';
        if (!file_exists($dir)) {
            mkdir($dir, 0777, true);
        }
     ...
     * @return string
     */
    static function buildImageVerify($length = 4, $mode = 1, $type = 'png', $width = 168, $height = 59, $verifyName = 'verify')
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            imagestring($im, 5, $i * 25 + 35, mt_rand(20, 40), $randval{$i}, $stringColor);
        }
